...
_CORETEMP_SHMEM_NAMES = (
    "CoreTempMappingObjectEx",
    "Local\\CoreTempMappingObjectEx",
    "Global\\CoreTempMappingObjectEx",
)
_CORETEMP_FMT = "<256I128I2I256f4f100sBB"
_CORETEMP_SIZE = struct.calcsize(_CORETEMP_FMT)


def read_cpu_temp_coretemp_shmem():
    """Highest CPU-core temperature in °C from Core Temp's shared memory, or
    None if Core Temp is not running. Reads CoreTempMappingObjectEx directly
    via mmap — no DLL, no admin rights."""
    if not IS_WINDOWS:
        return None
    import mmap
    mm = None
    for name in _CORETEMP_SHMEM_NAMES:
        try:
            mm = mmap.mmap(-1, _CORETEMP_SIZE, name, access=mmap.ACCESS_READ)
            break
        except OSError:
            continue
    if mm is None:
        return None
    try:
        fields = struct.unpack(_CORETEMP_FMT, mm[:_CORETEMP_SIZE])
        tjmax = fields[256]
        core_cnt = fields[384]
        temps = fields[386:642]
        fahrenheit = fields[647]
        delta_to_tjmax = fields[648]
        if core_cnt <= 0 or core_cnt > 256:
            return None
        best = None
...
class CpuSampler:
    ...
    def _sample_temp(self):
        ...
        try:
            ...
            if t is None:                                 # Core Temp shared memory
                t = read_cpu_temp_coretemp_shmem()
            self.temp = t
        except Exception:
            self.temp = None
