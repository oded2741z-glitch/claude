...
כבר רצה**: התוכנה מזהה אותן לבד ועוברת אליהן, בלי ללחוץ על כלום.
כתובות שהתקשרת אליהן (ושהתקשרו ממנה אליך) נשמרות
...
**שימו קובץ בשם `app_icon.ico`** (או `icon.ico`) בתיקייה הזאת לפני הבנייה —
זה האייקון שישמש גם את קובץ ה-EXE עצמו (שורת המשימות, סייר הקבצים) **וגם**
את החלון עצמו כשהתוכנה רצה, כי הקובץ נארז לתוך ה-EXE ונטען ממנו בזמן ריצה.
אין צורך לגעת בקוד — `app_icon.ico` מזוהה אוטומטית בכל מקום שהתוכנה עלולה
לרוץ ממנו: לצד `main.py` בהרצה מקובץ המקור, ומתוך ה-EXE הארוז.

אם לא שמתם שום קובץ אייקון, הבנייה מייצרת אייקון ברירת מחדל בעצמה (ריבוע
שחור פשוט) כדי שלא יישאר האייקון הכללי של PyInstaller.

⚠️ **הוא חייב להיות קובץ `.ico` אמיתי.** קובץ PNG ששינו לו את הסיומת ל-`.ico`
הוא הסיבה הנפוצה ביותר לכך ש**הבנייה נעצרת באמצע**: PyInstaller בודק את
החתימה בתוך הקובץ, ואם היא לא תואמת הוא מנסה להמיר עם Pillow — וכשהוא לא
מותקן, הבנייה נכשלת עם `ValueError`. הבנייה כאן בודקת את הקובץ מראש, כותבת
הודעה ברורה, וממשיכה עם האייקון המובנה במקום ליפול. אם אתם רוצים שההמרה
תעבוד בכל זאת: `pip install pillow`, או המירו את התמונה ל-`.ico` אמיתי.
...
## קובץ אחד (LANPhone_single.py)

כל הקוד קיים גם כ**קובץ בודד**, `LANPhone_single.py` (כ-3,650 שורות), אם נוח
לכם להעביר קובץ אחד במקום תיקייה:

```bat
python LANPhone_single.py
```

הוא עושה בדיוק את אותו דבר כמו `python main.py` ותומך באותן אפשרויות שורת
פקודה. עדיין צריך `numpy` ו-`sounddevice` מותקנים — הקובץ מאחד רק את הקוד
שלנו, לא את הספריות החיצוניות. (לקובץ שרץ בלי כלום מותקן, ראו את ה-EXE למעלה.)

⚠️ **הקובץ הזה נוצר אוטומטית — אל תערכו אותו ידנית.** מקור האמת הוא חבילת
`lanphone/`, והקובץ הבודד נבנה ממנה:

```bash
python tools/bundle.py           # יוצר מחדש
python tools/bundle.py --check   # בודק שהוא מעודכן
```

יש בדיקה אוטומטית שנכשלת אם שינו את החבילה ולא בנו מחדש, כך שהשניים לא
יכולים להתפצל בשקט.

**למה החבילה נשארה ולא נמחקה:** 165 הבדיקות מייבאות מודולים בודדים
(`lanphone.audio`, `lanphone.net`), וזה מה שמאפשר לבדוק את חוצץ ההשהיה או את
המרת קצב הדגימה בנפרד ובלי כרטיס קול. מיזוג הכול לקובץ אחד בתור המקור היה
מוחק את היכולת הזאת. ככה יש גם וגם: קובץ אחד להעברה, וחבילה נבדקת לפיתוח.

**איך המיזוג עובד:** הכלי לא עושה חיפוש-והחלפה על טקסט אלא עובד על **זרם
הטוקנים** של Python, כך שאזכור של שם מודול בתוך מחרוזת או הערה לא נוגעים בו.
ארבעה שמות מתנגשים בין המודולים (שלוש פונקציות `apply`, ועוד) ומקבלים שם חדש
בזמן המיזוג. בסוף יש מעבר אימות שקורא את התוצאה בחזרה כעץ תחביר ונכשל אם
נשארה הפניה כלשהי שלא תיפתר בזמן ריצה — כדי שלעולם לא ייכתב קובץ שבור.

`tools/bundle.py` דורש Python 3.12 ומעלה (התוכנה עצמה עדיין רצה על 3.9+):
לפני 3.12 מחרוזת f היא טוקן אחד אטום, ולכן `f"{net.foo()}"` לא נראה לכלי
והתוצאה הייתה יוצאת שבורה. הכלי מסרב לרוץ שם במקום להוציא קובץ פגום.
...
2. בכל מחשב בדקו שבשדות `Microphone` ו-`Headset / speaker` נבחרו האוזניות.
   אם חיברתם אותן אחרי שהתוכנה עלתה הן ייבחרו לבד תוך שנייה-שתיים.
...
**חיברתם אוזניות? הן פשוט נתפסות.** אין צורך ללחוץ על כלום — התוכנה בודקת כל
שנייה וחצי אם משהו השתנה, ותוך שנייה-שתיים עוברת לאוזניות. ביומן מופיע
`Now using: ...`. זה עובד גם כשהתוכנה סתם פתוחה וגם **באמצע שיחה**.

איך זה עובד:

* PortAudio קורא את רשימת ההתקנים פעם אחת בעלייה, וסריקה מחדש דורשת אתחול
  שלו — כלומר עצירה רגעית של השמע. לכן קודם נעשית בדיקה זולה מול Windows
  (`waveInGetNumDevs`/`waveOutGetNumDevs`) שרק סופרת התקנים; רק כשהמספר
  באמת השתנה מתבצעת הסריקה היקרה. ככה אפשר לזהות חיבור אוזניות גם תוך כדי
  שיחה, במחיר הפסקה של שבריר שנייה.
* כשהאפשרות `Always use a headset when one is connected` מסומנת (ברירת מחדל),
  אוזניות שהתחברו זה עתה מקבלות עדיפות על ההתקן הנוכחי. התקן חדש שאינו
  אוזניות — למשל מסך HDMI שהתחבר — **לא** גונב את הבחירה.
* גם בהפעלה: אם האוזניות כבר מחוברות והמשתמש לא בחר ידנית משהו אחר, הן
  נבחרות מיד.
* בחירה ידנית מהתפריט מכובדת ולא נדרסת, עד שמחברים אוזניות מחדש.
* אם האוזניות נשלפות באמצע שיחה, התוכנה מזהה שהזרם נפל, כותבת זאת ביומן,
  ועוברת להתקן הזמין הבא.
* `Refresh devices` נשאר ככפתור לסריקה מיידית, למקרה שמשהו לא נתפס.
* כשאין שיחה פעילה התוכנה לא מחזיקה את כרטיס הקול פתוח, כך שהחיבור נקלט
  בלי הפרעה.
...
* **סרגל כפתורים במקום תפריט.** Windows מצייר את שורת התפריטים בעצמו ומתעלם
  ...
  ו-`Quit` הם כפתורים רגילים בסרגל העליון.
...
* **שורת הכותרת שחורה ובלי אייקון.** גם אותה Windows מצייר, ו-Tk לא יכול לגעת
  ...
  מהכותרת באמצעות `WS_EX_DLGMODALFRAME`. במערכות אחרות הקוד לא עושה כלום.
* **אייקון החלון, לא רק של ה-EXE.** אם יש `app_icon.ico` (ראו "אייקון ל-EXE"
  למעלה), `lanphone/appicon.py` טוען אותו ומשתמש בו כאייקון החלון בפועל
  (`iconbitmap`) — לא רק כאייקון של קובץ ה-EXE בסייר הקבצים. בלי קובץ כזה
  הוא מצייר בקוד ריבוע שחור פשוט, כך שבכל מקרה לא נשארת הנוצה הכחולה של Tk.
...
באפור עמום. בפינה הימנית התחתונה יש סימן מים קטן — `oT`.
...
| אין קול בכלל | ביומן מופיעה שגיאת שמע? בדקו שההתקן הנכון נבחר, ולחצו `Refresh devices` אם האוזניות לא נתפסו לבד. הפעילו `Self test` כדי לבדוק כל צד בנפרד. |
...
165 בדיקות שמכסות המרת קצב דגימה, חוצץ ההשהיה, פורמט החבילות, מנוע ההקלטה וההשמעה (מול כרטיס קול מדומה), הממשק הגרפי, הכתובות השמורות, ושיחה
...
הגרסה הנוכחית היא **v1.0**. פירוט מלא של מה שנכנס בה נמצא ב-[CHANGELOG.md](CHANGELOG.md).
...
| `lanphone/appicon.py` | אייקון החלון, מצויר בקוד |
...
| `tools/bundle.py` | מיזוג החבילה לקובץ בודד |
| `LANPhone_single.py` | הקובץ הבודד (נוצר אוטומטית) |
