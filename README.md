...
אם לא שמתם שום קובץ אייקון, הבנייה **מייצרת** אייקון בעצמה (אותו אריח כהה עם
`oT` שרואים בשורת המשימות) — אין צורך לעשות כלום.

רוצים אייקון משלכם? שימו קובץ בשם **`app_icon.ico`** (או `icon.ico`) בתיקייה
הזאת לפני הבנייה.
...
* **אייקון משלנו במקום הנוצה של Tk.** `lanphone/appicon.py` מצייר בקוד אריח
  כהה עם הסימן `oT` בכתום — בלי קובץ תמונה נלווה — ומציב אותו כאייקון החלון,
  כך שגם בשורת המשימות לא נשארת הנוצה הכחולה.
...
148 בדיקות שמכסות המרת קצב דגימה, חוצץ ההשהיה, פורמט החבילות, מנוע ההקלטה וההשמעה (מול כרטיס קול מדומה), הממשק הגרפי, הכתובות השמורות, ושיחה
