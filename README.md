...
`lanphone/`, והקובץ הבודד נבנה ממנה במיזוג אוטומטי. שינוי בקוד צריך להיעשות
בחבילה, ואז לייצר את הקובץ הבודד מחדש.

הכלי שמייצר אותו (`tools/bundle.py`) הוסר מהתיקייה הזאת כדי להשאיר רק את מה
שנחוץ להרצה. הוא עדיין קיים בהיסטוריית git, ואפשר להחזיר אותו בשורה אחת:

```bash
git checkout 6bc66ba -- tools/
```

(מי שרק מריץ את התוכנה ולא משנה בה קוד — לא צריך אותו בכלל.)
...
לתוכנה יש 165 בדיקות אוטומטיות שמכסות המרת קצב דגימה, חוצץ ההשהיה, פורמט
החבילות, מנוע ההקלטה וההשמעה (מול כרטיס קול מדומה), הממשק הגרפי, הכתובות
השמורות, ושיחה מלאה מקצה לקצה — התקשרות, מענה, דחייה, "תפוס", ניתוק, פסק זמן
ובחירת איכות — הכול על ממשק ה-loopback, בלי צורך בכרטיס קול אמיתי.

תיקיית `tests/` הוסרה מהתיקייה הזאת כדי להשאיר רק את מה שנחוץ להרצה. הבדיקות
שמורות בהיסטוריית git, ואפשר להחזיר אותן ולהריץ:

```bash
git checkout 6bc66ba -- tests/
...
Run `python -m lanphone --list-devices` to check what the sound system sees. The
165-test suite and the single-file bundler live in git history rather than in
this directory; restore them with `git checkout 6bc66ba -- tests/ tools/`.
