...
class Video360App:
    def __init__(self, root):
        ...
        self.bars_visible = False; self.last_bar_activity = time.time()
        self.hud_enabled = True; self.anim = None; self.pano_rect = None
    ...
    def load_config(self):
        ...
        self.rtsp_history = []
        self.last_main_source = None
        self.tir_deadzone, self.tir_curve, self.tir_gain = 1.0, 1.5, 2.0
            ...
            try:
                with open(CONFIG_FILE, "r") as f:
                    ...
                    if len(lines) > 4 and lines[4].strip():
                        c = lines[4].split(',')
                        if len(c) >= 3: self.tir_deadzone, self.tir_curve, self.tir_gain = float(c[0]), float(c[1]), float(c[2])
            except: pass
    ...
    def save_config(self):
        try:
            with open(CONFIG_FILE, "w") as f: 
                ...
                f.write(f"{self.tir_deadzone},{self.tir_curve},{self.tir_gain}\n")
        except: pass
    ...
    def open_config(self):
        ...
        cfg_win.geometry("300x690") 
        cfg_win.configure(bg=ACCENT_COLOR)
        cfg_win.overrideredirect(True)
        rx, ry = self.root.winfo_x(), self.root.winfo_y()
        x = rx + (self.root.winfo_width() // 2) - 150; y = ry + (self.root.winfo_height() // 2) - 345
        ...
        tk.Label(frame, text="TrackIR Response:", fg=TEXT_COLOR, bg=BG_COLOR).pack(pady=(10,0))
        curve_scales = {}
        for key, label, lo, hi, res, val in (("dz", "Deadzone", 0, 5, 0.5, self.tir_deadzone), ("curve", "Curve", 1.0, 3.0, 0.1, self.tir_curve), ("gain", "Gain", 1.0, 4.0, 0.1, self.tir_gain)):
            row = tk.Frame(frame, bg=BG_COLOR); row.pack(pady=1)
            lbl = tk.Label(row, text=f"{label}: {val:g}", width=14, anchor="w", fg=TEXT_COLOR, bg=BG_COLOR); lbl.pack(side="left")
            sc = tk.Scale(row, from_=lo, to=hi, resolution=res, orient=tk.HORIZONTAL, length=120, bg=BG_COLOR, fg=TEXT_COLOR, troughcolor=BTN_COLOR, highlightthickness=0, showvalue=0, command=lambda v, l=lbl, n=label: l.config(text=f"{n}: {float(v):g}"))
            sc.set(val); sc.pack(side="left", padx=5)
            curve_scales[key] = sc
        ...
        def apply():
            ...
            self.tir_deadzone, self.tir_curve, self.tir_gain = float(curve_scales["dz"].get()), float(curve_scales["curve"].get()), float(curve_scales["gain"].get())
    ...
    def setup_ui(self):
        self.top_bar = tk.Frame(self.main_frame, bg=BG_COLOR, height=35); self.top_bar.pack_propagate(False)
        ...
        self.bottom_bar = tk.Frame(self.main_frame, bg=BG_COLOR, height=45)
        
        center_frame = tk.Frame(self.bottom_bar, bg=BG_COLOR)
        center_frame.pack(side="top", pady=7)
    ...
    def show_bars(self):
        self.last_bar_activity = time.time()
        if self.bars_visible: return
        self.top_bar.place(x=0, y=0, relwidth=1, height=35)
        self.bottom_bar.place(x=0, rely=1, relwidth=1, height=45, anchor="sw")
        self.top_bar.lift(); self.bottom_bar.lift()
        self.bars_visible = True

    def hide_bars(self):
        if not self.bars_visible: return
        self.top_bar.place_forget(); self.bottom_bar.place_forget()
        self.bars_visible = False

    def bars_tick(self):
        try:
            px = self.root.winfo_pointerx() - self.root.winfo_rootx(); py = self.root.winfo_pointery() - self.root.winfo_rooty()
            w, h = self.root.winfo_width(), self.root.winfo_height()
            if 0 <= px <= w and (0 <= py < 50 or h - 60 < py <= h): self.show_bars()
            elif time.time() - self.last_bar_activity > 2: self.hide_bars()
        except Exception: pass
        self.root.after(200, self.bars_tick)

    def toggle_hud(self): self.hud_enabled = not self.hud_enabled
    ...
    def reset_to_home(self):
        ...
        if self.input_mode == "MOUSE": target_yaw, target_pitch = self.home_yaw, self.home_pitch
        elif self.input_mode == "TRACKIR":
            ty, tp, tz = self.trackir.get_data(); target_yaw = self.home_yaw - self.apply_curve(ty); target_pitch = self.home_pitch - self.apply_curve(tp)
        else: return
        self.start_view_animation(self.wrap_delta(target_yaw - self.offset_yaw), target_pitch - self.offset_pitch)

    def apply_curve(self, angle):
        ref = 45.0; m = max(0.0, abs(angle) - self.tir_deadzone)
        return math.copysign(ref * self.tir_gain * (m / (ref - self.tir_deadzone)) ** self.tir_curve, angle)

    def wrap_delta(self, d): return ((d + 180) % 360) - 180 if self.view_mode == "360" else d

    def start_view_animation(self, d_yaw, d_pitch):
        self.anim = {"dy": d_yaw, "dp": d_pitch, "t0": time.time(), "ay": 0.0, "ap": 0.0}

    def step_view_animation(self):
        if not self.anim: return
        a = self.anim
        p = min(1.0, (time.time() - a["t0"]) / 0.5); e = p * p * (3 - 2 * p)
        self.offset_yaw += a["dy"] * e - a["ay"]; self.offset_pitch += a["dp"] * e - a["ap"]
        a["ay"], a["ap"] = a["dy"] * e, a["dp"] * e
        if p >= 1: self.anim = None
    ...
    def on_canvas_click(self, event):
        if self.pano_rect:
            x0, y0, sw, sh, rng = self.pano_rect
            if x0 <= event.x <= x0 + sw and y0 <= event.y <= y0 + sh:
                target = (0.5 - (event.x - x0) / sw) * rng
                self.start_view_animation(self.wrap_delta(target - self.yaw), 0)
                return
        self.pip_mouse_down(event)
    ...
    def draw_panorama(self, img, frame, w, h, src_w, src_h):
        self.pano_rect = None
        if not self.bars_visible or self.lens_mode == "Fisheye": return
        rng = {"360": 360, "180": 180, "120": 120}[self.view_mode]
        sw = min(640, int(w * 0.6)); sh = max(30, sw // 8)
        x0 = (w - sw) // 2; y0 = h - 50 - sh
        if sw < 50 or y0 < 0: return
        band = min(src_h, max(1, int(src_w * sh / sw))); top = (src_h - band) // 2
        strip = cv2.resize(frame[top:top + band], (sw, sh), interpolation=cv2.INTER_AREA)
        bw = max(4, int(self.current_fov / rng * sw)); bc = int((0.5 - self.yaw / rng) * sw)
        for off in ((-sw, 0, sw) if rng == 360 else (0,)):
            left = bc + off - bw // 2
            cv2.rectangle(strip, (left, 0), (left + bw, sh - 1), ON_BGR, 2)
        img[y0:y0 + sh, x0:x0 + sw] = strip
        cv2.rectangle(img, (x0 - 1, y0 - 1), (x0 + sw, y0 + sh), (51, 51, 51), 1)
        self.pano_rect = (x0, y0, sw, sh, rng)

    def draw_hud(self, img, w, h):
        c = ON_BGR; aa = cv2.LINE_AA
        heading = (-(self.yaw - self.home_yaw)) % 360
        cx = w // 2; ty = 50; half = int(w * 0.3)
        span = (self.current_fov / 2) * (half / (w / 2))
        cv2.line(img, (cx - half, ty + 14), (cx + half, ty + 14), c, 1, aa)
        for d in range(int(math.floor(heading - span)), int(math.ceil(heading + span)) + 1):
            if d % 5: continue
            x = int(cx + (d - heading) / span * half)
            big = d % 10 == 0
            cv2.line(img, (x, ty + 14), (x, ty + 14 - (10 if big else 5)), c, 1, aa)
            if big and abs(x - cx) > 20:
                label = str(d % 360); (tw, _), _ = cv2.getTextSize(label, HUD_FONT, 0.4, 1)
                cv2.putText(img, label, (x - tw // 2, ty), HUD_FONT, 0.4, c, 1, aa)
        cv2.fillPoly(img, [np.array([[cx, ty + 16], [cx - 5, ty + 24], [cx + 5, ty + 24]], np.int32)], c, aa)
        label = f"{int(round(heading)) % 360:03d}"; (tw, _), _ = cv2.getTextSize(label, HUD_FONT, 0.5, 1)
        cv2.putText(img, label, (cx - tw // 2, ty + 42), HUD_FONT, 0.5, c, 1, aa)

        elev = -self.pitch
        px = w - 50; pcy = h // 2; phalf = 80; prange = 30
        cv2.line(img, (px, pcy - phalf), (px, pcy + phalf), c, 1, aa)
        for d in range(int(math.floor(elev - prange)), int(math.ceil(elev + prange)) + 1):
            if d % 10: continue
            y = int(pcy - (d - elev) / prange * phalf)
            cv2.line(img, (px, y), (px + 8, y), c, 1, aa)
            cv2.putText(img, str(d), (px + 12, y + 4), HUD_FONT, 0.35, c, 1, aa)
        cv2.fillPoly(img, [np.array([[px - 2, pcy], [px - 10, pcy - 5], [px - 10, pcy + 5]], np.int32)], c, aa)
        label = f"{int(round(elev)):+d}"; (tw, _), _ = cv2.getTextSize(label, HUD_FONT, 0.45, 1)
        cv2.putText(img, label, (px - 14 - tw, pcy + 5), HUD_FONT, 0.45, c, 1, aa)

        cv2.putText(img, f"FOV {self.current_fov:.0f}", (15, ty + 18), HUD_FONT, 0.45, c, 1, aa)

    def process_frame_and_display(self, frame, src_w, src_h):
        self.step_view_animation()
        if self.input_mode == "TRACKIR":
            ty, tp, tz = self.trackir.get_data()
            self.yaw, self.pitch = self.apply_curve(ty)+self.offset_yaw, self.apply_curve(tp)+self.offset_pitch
        ...
        if win_w > 10:
            ...
            self.draw_panorama(final_img, frame, win_w, win_h, src_w, src_h)
            if self.hud_enabled: self.draw_hud(final_img, win_w, win_h)
