...
try:
    ...
    # --temps prints a text-only sensor report, so it has to keep working on
    # a machine without the tkinter package (headless server, trimmed
    # install). Only the GUI genuinely needs it.
    if "--temps" not in sys.argv[1:]:
        sys.exit("tkinter is missing. On Debian/Ubuntu run:  sudo apt install python3-tk")
    import types
    tk = tkfont = types.SimpleNamespace(Canvas=object, TclError=Exception)
...
def find_nvidia_smi():
    """Path to nvidia-smi.exe / nvidia-smi, or None.

    PATH first; on Windows the driver also drops it in System32 and, on
    older driver packages, in 'NVIDIA Corporation\\NVSMI' under Program
    Files — a PATH lookup misses those, which is the usual reason the GPU
    tile stays empty on a machine that clearly has an NVIDIA card."""
    exe = shutil.which("nvidia-smi") or shutil.which("nvidia-smi.exe")
    if exe:
        return exe
    if IS_WINDOWS:
        for raw in (r"%SystemRoot%\System32\nvidia-smi.exe",
                    r"%ProgramFiles%\NVIDIA Corporation\NVSMI\nvidia-smi.exe",
                    r"%ProgramW6432%\NVIDIA Corporation\NVSMI\nvidia-smi.exe",
                    r"%ProgramFiles(x86)%\NVIDIA Corporation\NVSMI\nvidia-smi.exe"):
...
def find_powershell():
    ...
    Falls back to the fixed System32 location: a trimmed PATH (common in
    services and in some frozen-app launchers) hides powershell.exe from
    `which`, and without it the WMI temperature query never runs."""
    if IS_WINDOWS:
        ...
        return p if "%" not in p and os.path.exists(p) else None
...
def print_temp_report():
    """--temps: print every temperature source and what it returned.

    A quick way to find out *why* a tile shows '\u2014' on a given machine,
    without opening the GUI."""
    def fmt(v):
        return "%.1f \u00b0C" % v if isinstance(v, (int, float)) else "not available"

    print("CPU / GPU temperature sources\n" + "-" * 46)

    t = read_cpu_temp()
    print("Linux sysfs (thermal_zone / hwmon)   CPU: %s" % fmt(t))

    cw, gw = read_temps_lhm_web()
    print("LibreHardwareMonitor web (:8085)     CPU: %-16s GPU: %s"
          % (fmt(cw), fmt(gw)))

    ps = find_powershell()
    if ps:
        cp, gp = read_temps_powershell(ps)
        print("PowerShell WMI (LHM / ACPI zone)     CPU: %-16s GPU: %s"
              % (fmt(cp), fmt(gp)))
        print("    host: %s" % ps)
    else:
        print("PowerShell WMI                       no powershell host found")

    nv = find_nvidia_smi()
    if nv:
        util, gt = read_gpu_stats(nv)
        print("nvidia-smi                           GPU: %-16s load: %s"
              % (fmt(gt), "%.0f%%" % util if util is not None else "n/a"))
        print("    exe:  %s" % nv)
    else:
