...
@server_sio.event
def update_screen_state(sid, data=None):
...
@server_sio.event
def request_snapshot(sid, data=None):
    if isinstance(data, dict):
        server_sio.emit('execute_snapshot', data)

@server_sio.event
def announce_viewer(sid, data=None):
    if not isinstance(data, dict):
        return
    target = data.get("target")
    if not target:
...
class DisplayController(ctk.CTk):
    def __init__(self):
        ...
        if os.path.exists(SCENES_FILE):
            try:
                ...
                self.log(f"[ERROR] Failed to load scenes: {e}")
    ...
    def load_scene(self):
        ...
        for target in list(self.app_state.keys()):
            if target not in saved_state:
                empty_payload = dict(EMPTY_PAYLOAD)
                self.app_state[target].update(empty_payload)
                if sio.connected:
        ...
        if saved_state:
            if not self.current_stream_target or self.current_stream_target not in saved_state:
                first_target = next(iter(saved_state))
    ...
    def _write_scenes_file(self):
        try:
            with open(SCENES_FILE, "w", encoding="utf-8") as f:
                json.dump(self.scenes_data, f, ensure_ascii=False, indent=4)
            return True
        except Exception as e:
            self.log(f"[ERROR] Failed to write scenes file: {e}")
            return False
    ...
    def take_snapshot(self, idx):
        if not self.current_stream_target:
            self.log("[WARNING] Select a screen first to take a snapshot.")
            return
    ...
    def _refresh_blackout_button(self):
        self.is_blackout = any(
            isinstance(s, dict) and s.get("blackout") == "True"
            for s in self.app_state.values()
        )
        if self.is_blackout:
    ...
    def toggle_blackout(self):
        new_value = not self.is_blackout
        if new_value:
        ...
        for lbl, data in self.nodes.items():
            if data["type"] != "GPU":
                if lbl not in self.app_state:
                    self.app_state[lbl] = {}
                self.app_state[lbl]['blackout'] = str(new_value)
                if sio.connected:
                    sio.emit("update_screen_state", {"target": lbl, "payload": {"blackout": str(new_value)}})

        if not new_value:
            for lbl, state in self.app_state.items():
                if isinstance(state, dict) and state.get("blackout") == "True":
                    state["blackout"] = "False"
                    if sio.connected:
                        sio.emit("update_screen_state", {"target": lbl, "payload": {"blackout": "False"}})
    ...
    def toggle_viewer_checkbox(self, lbl, layout=4):
        ...
        return lbl in self.active_viewers
    ...
    def _checkbox_rects(self, x2, y1):
        x1 = x2 - 18
        return [
            (4, x1, y1 + 5, x1 + CB_SIZE, y1 + 5 + CB_SIZE),
            (2, x1, y1 + 20, x1 + CB_SIZE, y1 + 20 + CB_SIZE),
            (1, x1, y1 + 35, x1 + CB_SIZE, y1 + 35 + CB_SIZE),
        ]

    def on_node_click(self, event, lbl, coords):
        if self.nodes[lbl]["type"] != "GPU":
            for layout, cx1, cy1, cx2, cy2 in self._checkbox_rects(coords['x2'], coords['y1']):
                if cx1 <= event.x <= cx2 and cy1 <= event.y <= cy2:
                    if self.is_controller_on_screen(lbl):
                        self.log(f"[WARNING] Cannot launch on '{lbl}'.")
                    elif self.toggle_viewer_checkbox(lbl, layout=layout):
                        self.selected_targets = {lbl}
                        self.sync_stream_ui_from_state(lbl)
                    elif lbl in self.selected_targets or self.current_stream_target == lbl:
                        self.selected_targets.clear()
                        self._disable_stream_controls()
                    self.draw_graph()
                    return
    ...
    def draw_graph(self):
        ...
        for lbl, d in node_coords.items():
            ...
            if dev_type == "GPU":
                ...
                is_running = lbl in self.active_viewers
                current_layout = self.screen_layouts.get(lbl, 4)
                rects = self._checkbox_rects(d['x2'], d['y1'])

                if is_host:
                    first = rects[0]
                    last = rects[-1]
                    self.canvas.create_rectangle(first[1], first[2], last[3], last[4], fill="#333333", outline="#666666", tags=safe_tag)
                    self.canvas.create_line(first[1]+2, first[2]+2, last[3]-2, last[4]-2, fill="#888888", width=2, tags=safe_tag)
                    self.canvas.create_line(last[3]-2, first[2]+2, first[1]+2, last[4]-2, fill="#888888", width=2, tags=safe_tag)
                else:
                    for layout, cx1, cy1, cx2, cy2 in rects:
                        active = is_running and current_layout == layout
                        fill = COLORS["ACCENT"] if active else COLORS["BG_MAIN"]
                        text_color = "black" if active else "white"
                        self.canvas.create_rectangle(cx1, cy1, cx2, cy2, fill=fill, outline="white", tags=safe_tag)
                        self.canvas.create_text(cx1 + CB_SIZE/2, cy1 + CB_SIZE/2, text=str(layout), fill=text_color, font=("Consolas", 8, "bold"), tags=safe_tag)
